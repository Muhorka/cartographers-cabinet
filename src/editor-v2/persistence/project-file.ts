...
function validateProjectRelations(project: EditorProject, context: z.RefinementCtx) {
  ...
  for (const [index, document] of project.constructions.entries()) {
    ...
    for (const transitionValue of document.transitions) {
      ...
      for (const levelId of referenced) if (!placeIds.has(levelId)) context.addIssue({ code: "custom", message: `Vertical connection references a missing level: ${levelId}`, path: ["constructions", index, "transitions"] });
    }
  }
}
