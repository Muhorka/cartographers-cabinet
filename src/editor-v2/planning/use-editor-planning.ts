...
import { useEditorTransaction } from "../components/use-editor-transaction";
...
export function useEditorPlanning({ session, snapshot, selections, locale, refresh, onSplit }: { session?: EditorSession; snapshot?: EditorSessionState; selections: EditableSelection[]; locale: EditorLocale; refresh(): void; onSplit?(): void }) {
  ...
  const { commit: commitTransaction } = useEditorTransaction(session, refresh, (failure) => setNotice(failure ? workbenchCopy[locale].editingStatus.blocked[failure] : undefined));
