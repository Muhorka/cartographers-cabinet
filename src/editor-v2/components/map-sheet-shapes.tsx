...
export function PlaceShape({ place, mode, prefix, transform, labelMatrix, labelCollision, selectionLayer, selectable = false, allowLockedSelection = false, viewportZoom = 1, resizeHandleSize = 5, showResizeHandles = false, selected, showArea = false, units = "metric", onSelect, onOpen }: { place: PlaceNode; mode: "active" | "child" | "context" | "descendant"; prefix: string; transform?: string; labelMatrix?: AffineMatrix; labelCollision?: LabelCollisionRegistry; selectionLayer?: WorkLayerId; selectable?: boolean; allowLockedSelection?: boolean; viewportZoom?: number; resizeHandleSize?: number; showResizeHandles?: boolean; selected?: boolean; showArea?: boolean; units?: "metric" | "imperial"; onSelect?(additive?: boolean): void; onOpen?(): void }) {
  ...
  const label = regionLabelLayout(mapLabelWithArea(place.name, mapRegionArea(place.boundary), units, showArea), place.boundary, viewportZoom, place.kind === "location" || place.kind === "custom", { obstacles: labelCollision?.obstaclesFor(labelMatrix) });
  if (showLabel) labelCollision?.register(label?.kind === "inside" ? label : undefined, labelMatrix);
...
export function ElementShape({ element, prefix, viewportZoom, pointRadius, resizeHandleSize, opacity, selectable, showResizeHandles, selected, showArea = false, units = "metric", labelObstacles, labelMatrix, labelCollision, onSelect, onNoteTextChange }: { element: DrawingElement; prefix: string; viewportZoom: number; pointRadius: number; resizeHandleSize: number; opacity: number; selectable: boolean; showResizeHandles: boolean; selected: boolean; showArea?: boolean; units?: "metric" | "imperial"; labelObstacles?: readonly LabelObstacle[]; labelMatrix?: AffineMatrix; labelCollision?: LabelCollisionRegistry; onSelect?(additive?: boolean): void; onNoteTextChange?(id: string, text: string): void }) {
