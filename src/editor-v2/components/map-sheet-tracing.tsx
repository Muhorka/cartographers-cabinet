...
export const MapSheetTracing = memo(function MapSheetTracing({ project, activePlaceId, prefix, copy, viewportZoom, opacity }: { project: EditorProject; activePlaceId: string; prefix: string; copy: MapSheetCopy; viewportZoom: number; opacity: number }) {
  ...
  const shape = (place: (typeof project.places)[number], mode: "active" | "child" | "context" | "descendant") => <PlaceShape key={place.id} place={place} mode={mode} prefix={prefix} viewportZoom={viewportZoom} labelMatrix={relativePlaceMatrix(project, activePlaceId, place.id)} labelCollision={labelCollision} transform={place.id === activePlaceId ? undefined : matrixAttribute(relativePlaceMatrix(project, activePlaceId, place.id))}/>;
  return <g className={styles.tracing} style={{ opacity }} data-tracing-overlay="true" aria-hidden="true">
    <MapSheetElements project={project} activePlaceId={activePlaceId} terrain prefix={prefix} selected={new Set()} movingIds={new Set()} selectionEditing={false} sketchVisible sketchOpacity={1} viewportZoom={viewportZoom} labelCollision={labelCollision}/>
    ...
    {network && document && <MapSheetConstruction project={project} document={document} network={network} owner={owner} prefix={prefix} copy={copy} selectedIds={new Set()} viewportZoom={viewportZoom} roomView={roomView} roomScope={roomScope} activeGesture={false} selectionEditing={false} movingIds={new Set()} labelMatrix={relativePlaceMatrix(project, activePlaceId, owner?.id ?? activePlaceId)} labelCollision={labelCollision}/>}
    <MapSheetElements project={project} activePlaceId={activePlaceId} terrain={false} prefix={prefix} selected={new Set()} movingIds={new Set()} selectionEditing={false} sketchVisible sketchOpacity={1} viewportZoom={viewportZoom} labelCollision={labelCollision}/>
  </g>;
});
