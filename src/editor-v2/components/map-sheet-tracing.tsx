...
export const MapSheetTracing = memo(function MapSheetTracing({ project, activePlaceId, prefix, copy, viewportZoom, opacity }: { project: EditorProject; activePlaceId: string; prefix: string; copy: MapSheetCopy; viewportZoom: number; opacity: number }) {
  ...
  const labelPlan = createSceneLabelPlan(project, activePlaceId, groups, document, network, owner, viewportZoom, false, project.measureSettings.units, true, new Set());
  ...
  return <g className={styles.tracing} style={{ opacity }} data-tracing-overlay="true" aria-hidden="true">
    <MapSheetElements project={project} activePlaceId={activePlaceId} terrain prefix={prefix} selected={new Set()} movingIds={new Set()} selectionEditing={false} sketchVisible sketchOpacity={1} viewportZoom={viewportZoom} labelPlan={labelPlan}/>
    ...
    {network && document && <MapSheetConstruction project={project} document={document} network={network} owner={owner} prefix={prefix} copy={copy} selectedIds={new Set()} viewportZoom={viewportZoom} roomView={roomView} roomScope={roomScope} activeGesture={false} selectionEditing={false} movingIds={new Set()} labelPlan={labelPlan}/>}
    <MapSheetElements project={project} activePlaceId={activePlaceId} terrain={false} prefix={prefix} selected={new Set()} movingIds={new Set()} selectionEditing={false} sketchVisible sketchOpacity={1} viewportZoom={viewportZoom} labelPlan={labelPlan}/>
  </g>;
});
