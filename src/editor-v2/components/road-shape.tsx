...
export function RoadShape({ element, prefix, zoom, handles, selected, showArea = false, units = "metric", labelObstacles, labelPlan }: { element: DrawingElement; prefix: string; zoom: number; handles: boolean; selected: boolean; showArea?: boolean; units?: "metric" | "imperial"; labelObstacles?: readonly LabelObstacle[]; labelPlan?: LabelLayoutPlan }) {
  ...
  const clipId = `${filter}-clip`; const label = (labelPlan?.get(`element:${element.belongsToId}:${element.id}`) as ReturnType<typeof regionLabelLayout> | undefined) ?? regionLabelLayout(mapLabelWithArea(element.name, mapRegionArea(shape), units, showArea), shape, zoom, false, { obstacles: labelObstacles });
  return <>
