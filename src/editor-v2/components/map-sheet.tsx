...
import { useEffect, useId, useMemo, useRef, useState, type KeyboardEvent, type MouseEvent as ReactMouseEvent, type PointerEvent, type WheelEvent } from "react";
...
import { MapGesturePreview } from "./map-gesture-preview"; import { MapSheetConstruction } from "./map-sheet-construction";
...
import { connectedTransitionsForView, constructionPlaceForView, fitViewportToRegion, matrixAttribute, panViewport, pointsPath, relativePlaceMatrix, roomEditingScope, viewportRegion, visiblePlaceGroups, zoomViewport } from "./map-sheet-geometry";
import styles from "./map-sheet.module.css"; import { Compass, PlaceShape } from "./map-sheet-shapes";
...
import { MapSheetElements } from "./map-sheet-elements"; import { MapSheetTracing } from "./map-sheet-tracing"; import { useDrawingKeyboardConfirmation } from "./use-drawing-keyboard-confirmation"; import { preferredSelectable } from "./map-selection-target";
import { previewPlaceMatrix, previewRegionResize, previewRegionVertex, previewWallEndpoint } from "./map-resize-preview"; import { MapGrid } from "./map-grid";
...
export function MapSheet({ pointPicker, storyOverlay, rotationControl, project, activePlaceId, viewport, copy, selectedIds = [], draftStrokes = [], gestureDraft, sheetSize = { width: 1000, height: 700 }, interaction, selectionEditing = false, selectionOnly = false, outlineEditing = false, selectionMode = "direct", selectionLayerId, sketchVisible = true, sketchOpacity = .75, eraserSize = 10, gapClosingEnabled = false, gapClosingTolerance = 14, tracingProject, tracingOpacity = .4, onSelect, onSelectMany, onOpenPlace, onClearSelection, onDeleteSelected, onCancelDrawing, onViewportChange, onGesture, onGestureDraftChange, onMeasureSettingsChange, onNoteTextChange, onMoveSelection, onMoveWallEndpoint, onResizeOpening, onResizeTransition, onResizeElement, onResizeSurface, onResizePlace, onMoveElementVertex, onMoveSurfaceVertex, onMovePlaceVertex, nodeInsertion }: MapSheetProps) {
  ...
  const displayedProject = resizeProjectPreview ?? project; const groups = useMemo(() => visiblePlaceGroups(displayedProject, activePlaceId), [displayedProject, activePlaceId]); const selected = new Set(selectedIds);
  const movingIds = new Set(movePreview ? selected.has(movePreview.selection.id) ? selectedIds : [movePreview.selection.id] : []);
  const constructionOwner = constructionPlaceForView(displayedProject, activePlaceId); const roomView = groups.active?.kind === "room";
  const activeConstruction = displayedProject.constructions.find(({ id }) => id === constructionOwner?.constructionId); const contextTransitions = useMemo(() => constructionOwner ? connectedTransitionsForView(displayedProject, constructionOwner.id, activeConstruction?.id) : [], [activeConstruction?.id, constructionOwner, displayedProject]);
  const network = useMemo(() => activeConstruction ? constructionNetwork(activeConstruction.walls, activeConstruction.enclosure) : undefined, [activeConstruction]); const roomScope = useMemo(() => roomView ? roomEditingScope(groups.active, activeConstruction, network) : {}, [activeConstruction, groups.active, network, roomView]);
  ...
  function zoom(event: WheelEvent<SVGSVGElement>) {
    if (!onViewportChange) return; event.preventDefault(); const bounds = event.currentTarget.getBoundingClientRect();
    ...
    onViewportChange(zoomViewport(viewport, Math.exp(-event.deltaY * 0.0015), anchor, { x: sheetSize.width / 2, y: sheetSize.height / 2 }));
  }
  ...
  const placeTransform = (placeId: string) => matrixAttribute(previewPlaceMatrix(displayedProject, activePlaceId, placeId, movingIds.has(placeId) ? movePreview?.delta : undefined));
  ...
  return <div className={styles.sheet} data-drawing={activeGesture || nodeInsertion?.active ? "true" : undefined}>
    ...
    <svg {...insertion.handlers} className={styles.canvas} style={activeGesture || pointPicker ? { cursor: "crosshair" } : undefined} viewBox={`0 0 ${sheetSize.width} ${sheetSize.height}`} role="img" aria-label={copy.ariaLabel} tabIndex={0} onPointerDownCapture={(event) => { if (!capturePointPointer(event, pointPicker)) insertion.handlers.onPointerDownCapture(event); }} onClickCapture={(event) => { if (captureMapPoint(event, pointPicker, viewport, sheetSize)) return; insertion.handlers.onClickCapture(event); if (!nodeInsertion?.active) suppressDrawingSelection(event); }} onContextMenu={(event) => { if (pointPicker) { event.preventDefault(); pointPicker.cancel(); return; } if (!activeGesture) return; event.preventDefault(); showGesture(); lastPenClick.current = undefined; onCancelDrawing?.(); }} onDoubleClick={() => activeGesture && isPolygonGesture(activeGesture) && finishMultiClick()} onPointerDown={beginPan} onPointerMove={continuePan} onPointerUp={finishPan} onPointerCancel={cancelPointer} onWheel={zoom} onKeyDown={keyNavigation}>
      ...
      <g transform={mapTransform}>
        ...
        <g className={styles.contextTerrain}><MapSheetElements onNoteTextChange={selectionEditing || activeGesture === "note" ? onNoteTextChange : undefined} showArea={displayedProject.measureSettings.showRoomAreas} units={displayedProject.measureSettings.units} project={displayedProject} activePlaceId={activePlaceId} terrain prefix={prefix} selected={selected} movingIds={movingIds} movingTransform={movingTransform} selectionEditing={selectionEditing} selectionOnly={selectionOnly} selectionLayerId={selectionLayerId} sketchVisible={sketchVisible} sketchOpacity={sketchOpacity} viewportZoom={viewport.zoom} onSelect={activeGesture ? undefined : onSelect}/></g>
        <g className={styles.context} aria-hidden="true">{groups.context.filter(({ kind }) => kind !== "level").map((place) => <PlaceShape showArea={displayedProject.measureSettings.showRoomAreas} units={displayedProject.measureSettings.units} key={place.id} place={place} transform={matrixAttribute(relativePlaceMatrix(displayedProject, activePlaceId, place.id))} mode="context" prefix={prefix} viewportZoom={viewport.zoom}/>)}</g>
        {groups.active?.boundary && <PlaceShape showArea={displayedProject.measureSettings.showRoomAreas} units={displayedProject.measureSettings.units} place={groups.active} mode="active" prefix={prefix} viewportZoom={viewport.zoom} selectable={selectionEditing && outlineEditing || selectionOnly} allowLockedSelection={selectionOnly} resizeHandleSize={5 / viewport.zoom} showResizeHandles={outlineEditing && !activeConstruction} selected={selected.has(groups.active.id)} onSelect={activeGesture || selectionEditing && !outlineEditing ? undefined : (additive) => onSelect?.({ kind: "place", id: groups.active!.id }, additive)}/>}
        <g>{groups.children.filter(({ kind }) => kind !== "room" && !(groups.active?.kind === "building" && kind === "level")).map((place) => { const layer = place.kind === "building" ? "buildings" : "boundaries"; const selectable = selectionEditing || selectionOnly; return <PlaceShape showArea={displayedProject.measureSettings.showRoomAreas} units={displayedProject.measureSettings.units} key={place.id} place={place} transform={placeTransform(place.id)} mode="child" prefix={prefix} selectionLayer={layer} selectable={selectable} allowLockedSelection={selectionOnly} viewportZoom={viewport.zoom} resizeHandleSize={5 / viewport.zoom} showResizeHandles={outlineEditing && selectionEditing && selected.has(place.id)} selected={selected.has(place.id)} onSelect={activeGesture ? undefined : (additive) => onSelect?.({ kind: "place", id: place.id }, additive)} onOpen={activeGesture || sourceSelection ? undefined : () => onOpenPlace?.(place.id)}/>; })}</g>
        <g className={styles.descendants} aria-hidden="true">{groups.descendants.map((place) => <PlaceShape showArea={displayedProject.measureSettings.showRoomAreas} units={displayedProject.measureSettings.units} key={place.id} place={place} transform={matrixAttribute(relativePlaceMatrix(displayedProject, activePlaceId, place.id))} mode="descendant" prefix={prefix} viewportZoom={viewport.zoom}/>)}</g>
        <MapSheetSurfaces showArea={displayedProject.measureSettings.showRoomAreas} units={displayedProject.measureSettings.units} project={displayedProject} activePlaceId={activePlaceId} prefix={prefix} selected={selected} movingIds={movingIds} movingTransform={movingTransform} selectionEditing={selectionEditing && selectionLayerId === "construction"} selectionOnly={selectionOnly} viewportZoom={viewport.zoom} onSelect={(id, additive) => onSelect?.({ kind: "surface", id }, additive)}/>
        {network && activeConstruction && <MapSheetConstruction project={displayedProject} document={activeConstruction} network={network} owner={constructionOwner} prefix={prefix} copy={copy} selectedIds={selected} viewportZoom={viewport.zoom} roomView={roomView} roomScope={roomScope} activeGesture={Boolean(activeGesture)} selectionEditing={selectionEditing} selectionOnly={selectionOnly} selectionLayerId={selectionLayerId} movingIds={movingIds} moveDelta={movePreview?.delta} openingWidthPreview={openingWidthPreview} onSelect={onSelect} onOpenPlace={onOpenPlace} contextTransitions={contextTransitions}/>}
        <MapSheetElements onNoteTextChange={selectionEditing || activeGesture === "note" ? onNoteTextChange : undefined} showArea={displayedProject.measureSettings.showRoomAreas} units={displayedProject.measureSettings.units} project={displayedProject} activePlaceId={activePlaceId} terrain={false} prefix={prefix} selected={selected} movingIds={movingIds} movingTransform={movingTransform} selectionEditing={selectionEditing} selectionOnly={selectionOnly} selectionLayerId={selectionLayerId} sketchVisible={sketchVisible} sketchOpacity={sketchOpacity} viewportZoom={viewport.zoom} onSelect={activeGesture ? undefined : onSelect}/>
        {storyOverlay}
        {tracingProject && <MapSheetTracing project={tracingProject} activePlaceId={activePlaceId} prefix={`${prefix}-tracing`} copy={copy} viewportZoom={viewport.zoom} opacity={tracingOpacity}/>}
        ...
        {!groups.active && <text className={styles.empty} x={viewport.center.x} y={viewport.center.y}>{copy.empty}</text>}
      </g>
