  ...
  return <g className={`${styles.construction}${roomView ? ` ${styles.roomViewConstruction}` : ""}`}>
    {!roomView && network.faces.map((face) => {
      ...
      return <g key={room.id} className={styles.roomTarget} style={interactive || canNavigate ? undefined : { pointerEvents: "none" }} transform={translated(movingIds.has(room.id))} data-selectable={selectable ? "true" : undefined} data-selection-layer={selectable ? "construction" : undefined} data-selection-kind={selectable ? "room" : undefined} data-selection-id={selectable ? room.id : undefined} role={interactive || canNavigate ? "button" : undefined} tabIndex={interactive || canNavigate ? 0 : undefined} aria-label={interactive || canNavigate ? room.name : undefined} onClick={interactive ? (event) => { event.stopPropagation(); onSelect?.({ kind: "room", id: room.id }, additiveSelection(event)); } : undefined} onDoubleClick={canNavigate && !activeGesture ? (event) => { event.stopPropagation(); onOpenPlace?.(room.id); } : undefined} onKeyDown={(event) => { if (event.key === "Enter" && canNavigate && !activeGesture) { event.preventDefault(); onOpenPlace?.(room.id); } else activateByKeyboard(event, () => { if (interactive) onSelect?.({ kind: "room", id: room.id }); }); }}>
        ...
        <path d={roomPath(face)} fillRule="evenodd" style={appearance ? { fill: appearance.fillColor, fillOpacity: appearance.fillOpacity } : undefined} className={`${styles.room}${selectedIds.has(room.id) ? ` ${styles.selected}` : ""}`}/>
        {selectedIds.has(room.id) && <path d={roomPath(face)} fillRule="evenodd" className={styles.roomSelection} aria-hidden="true"/>}
    ...
    {document.walls.filter((wall) => wall.visible !== false).map((wall) => {
      ...
      return <g key={wall.id} transform={translated(movedWallIds.has(wall.id))} className={`${selectedIds.has(wall.id) ? styles.selectedWall : ""}${roomView && !wallSelectable ? ` ${styles.contextWall}` : ""}`}>
        ...
        <line className={styles.wallHit} x1={wall.start.x} y1={wall.start.y} x2={wall.end.x} y2={wall.end.y} data-selectable={wallSelectable ? "true" : undefined} data-selection-layer={wallSelectable ? "construction" : undefined} data-selection-kind={wallSelectable ? "wall" : undefined} data-selection-id={wallSelectable ? wall.id : undefined} role={wallSelectable ? "button" : undefined} tabIndex={wallSelectable ? 0 : undefined} aria-label={wallSelectable ? wall.id : undefined} onClick={wallSelectable ? (event) => { event.stopPropagation(); onSelect?.({ kind: "wall", id: wall.id }, additiveSelection(event)); } : undefined} onKeyDown={wallSelectable ? (event) => activateByKeyboard(event, () => onSelect?.({ kind: "wall", id: wall.id })) : undefined}/>
