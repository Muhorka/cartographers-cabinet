...
export function WorkbenchToolbox({ state, copy, availableLayerIds, availableSubjectIds, boundaryName, boundaryEditing, cutoutActive = false, addOutlineActive = false, canCutout = false, canAddOutline = canCutout, outlineInstrumentId, onOutlineInstrumentChange, collapsed, canUndo, canRedo, canClearLayer = true, clearLayerLabel, sketchVisible, sketchOpacity, eraserSize = 10, gapClosingEnabled = false, gapClosingTolerance = 14, pencilSmoothing = .25, selectionActions, onChange, onBoundaryEditing, onCutoutActive, onAddOutlineActive, onUndo, onRedo, onClearLayer, onCollapsed, onSketchVisible, onSketchOpacity, onEraserSize, onGapClosingEnabled, onGapClosingTolerance, onPencilSmoothing }: {
  ...
  return <section className={`${styles.case}${collapsed ? ` ${styles.collapsed}` : ""}`} aria-label={copy.ariaLabel}>
    ...
    {!collapsed && <>
      ...
      <div className={styles.instrumentBand}>
        <div className={styles.instrumentRail} role="toolbar" aria-label={copy.chooseInstrument}>
          ...
          {instrumentId === "pencil" && <label className={styles.pencilSmoothing}><span>{copy.pencilSmoothing}</span><input aria-label={copy.pencilSmoothing} type="range" min="0" max="1" step="0.05" value={pencilSmoothing} onChange={(event) => onPencilSmoothing?.(Number(event.currentTarget.value))}/><output>{Math.round(pencilSmoothing * 100)}%</output></label>}
          {canCleanLines && <div className={styles.lineCleanup}><button type="button" className={gapClosingEnabled ? styles.activeCleanup : undefined} aria-pressed={gapClosingEnabled} title={copy.closeGaps} onClick={() => onGapClosingEnabled?.(!gapClosingEnabled)}><ToolGlyph id="close-gaps"/><span>{copy.closeGaps}</span></button><label><span>{copy.closeGapsStrength}</span><input aria-label={copy.closeGapsStrength} type="range" min="4" max="30" step="1" value={gapClosingTolerance} disabled={!gapClosingEnabled} onChange={(event) => onGapClosingTolerance?.(Number(event.currentTarget.value))}/><output>{Math.round(gapClosingTolerance)} px</output></label></div>}
        </div>
