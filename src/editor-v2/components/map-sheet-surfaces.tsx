...
export function MapSheetSurfaces({ project, activePlaceId, prefix, selected, agentFocused = emptyAgentFocus, movingIds, movingTransform, selectionEditing, selectionOnly = false, viewportZoom, showArea = false, units = "metric", labelPlan, onSelect }: {
  ...
  return visible.map(({ surface, depth }) => {
    const editable = depth === 0 && selectionEditing; const selectable = depth !== undefined && (selectionOnly || editable); const surfaceSelection = { kind: "surface" as const, id: surface.id }; const key = selectionKey(surfaceSelection); const isSelected = selected.has(key); const isAgentFocused = agentFocused.has(key);
