...
import { TransitionCreationDialog } from "./transition-creation-dialog"; import { LegalMarginalia } from "./legal-marginalia";
...
export function EditorWorkbench() {
  ...
  return <main className={styles.workshop}>
    <header className={styles.masthead}><div><small>{locale === "pl" ? "PRACOWNIA PRZESTRZENNEGO ŚWIATOTWÓRSTWA" : "A SPATIAL WORLDBUILDING WORKSHOP"}</small><h1>{copy.title}</h1><p>{copy.strapline}</p></div><button type="button" className={styles.language} onClick={changeLocale}>{copy.language}</button></header>
