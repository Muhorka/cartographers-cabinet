...
import { createProjectAtScale, type StartingScale } from "../model/starter-project";
...
import { nextMapSelection, preferredCutoutTarget, viewportFor } from "./workbench-helpers";
...
import { installLoadedWorkbenchProject } from "./workbench-project-install";
import { createOverlapDepartureRequest } from "./workbench-overlap-departure";
...
export function EditorWorkbench() {
  ...
  const [expandedIds, setExpandedIds] = useState<Set<string>>(new Set()); const [selections, setSelections] = useState<MapSelection[]>([]); const selectedIds = useMemo(() => selections.map(selectionKey), [selections]); const [agentFocus, setAgentFocus] = useState<MapSelection[]>([]); const agentFocusedIds = useMemo(() => agentFocus.map(selectionKey), [agentFocus]); const [toolboxCollapsed, setToolboxCollapsed] = useState(false);
  ...
  const saveConflictError = autosave.saveConflict ? (locale === "pl" ? "Ten projekt zmieniono lub usunięto w innej karcie. Otwórz go ponownie przed dalszą pracą." : "This project was changed or deleted in another tab. Reopen it before continuing.") : undefined; const visibleOperationError = saveConflictError ?? operationError;
  const [bootError, setBootError] = useState<string>(); const [recoveryRecords, setRecoveryRecords] = useState<ProjectLibraryRecoveryRecord[]>([]); const workbenchRefresh = useWorkbenchSessionRefresh({ session, snapshot, setSnapshot, setSelections, setInspectorFocus: setInspectorTarget, setViewport, setExpandedIds, setCutoutActive, setAddOutlineActive });
  const installLoadedProject = (loaded: Awaited<ReturnType<typeof restoreWorkbenchProject>>) => { setAgentFocus([]); installLoadedWorkbenchProject(loaded, { setSession, setSnapshot, setSelections, setViewport, setPendingDeleteId, setPendingPlaceDeleteId, setPendingClearLayer, setPendingClearCategory, setPendingOverlapDeparture, setDismissedOverlapSignature, overlapContinuation, setInspectorTarget, setExpandedIds, setSketchVisible, setSketchOpacity, setEraserSize, setGapClosingEnabled, setGapClosingTolerance }); };
  ...
  useEffect(() => { let cancelled = false;
    ...
    }).catch((error: unknown) => { if (!cancelled) setBootError(safePersistenceError(error).reason); });
  ...
  function refresh(nextSession = session) { workbenchRefresh.refresh(nextSession); }
  ...
  function openPlace(placeId: string) { drawing.requestAfterDraft(() => requestAfterOverlap((replacementId) => commitOpenPlace(replacementId ?? placeId), placeId)); }
  ...
  function commitOpenPlace(placeId: string) {
    if (session) openWorkbenchPlace(session, placeId, { inspect: setInspectorTarget, expand: (ids) => setExpandedIds((current) => new Set([...current, ...ids])), clearSelection: () => setSelections([]), setViewport, refresh: () => refresh() });
  }
  const requestAfterOverlap = createOverlapDepartureRequest({ session, continuation: overlapContinuation, setPendingOverlapDeparture, setDismissedOverlapSignature });
  ...
  function addLevel(buildingId: string, position: "above" | "below") {
    ...
    setExpandedIds((current) => new Set([...current, buildingId, id])); commitOpenPlace(id);
  }
  ...
  function commitAddBroaderMap(placeId: string, desiredKind: MapLevelKind, chosenName?: string) {
    if (!session || !snapshot) return;
    let result: ReturnType<typeof addMapLevel>;
    if (!workbenchTransaction.commit(`wrap:${placeId}:${desiredKind}`, (current) => {
      result = addMapLevel(current, placeId, desiredKind, chosenName, locale, { createId: () => crypto.randomUUID() });
      return result?.project ?? current;
    })) return;
    const created = result; if (!created) return;
    setExpandedIds((current) => new Set([...current, ...created.addedIds, created.openedId])); commitOpenPlace(created.openedId);
  }
  async function createProject(input?: { name: string; scale: StartingScale }) { const name = (input?.name ?? draftName).trim(); if (!name) return; try { const created = await saveProject(createProjectAtScale(crypto.randomUUID(), name, locale, input?.scale ?? startingScale)); setProjects((current) => [created, ...current]); setDraftName(""); setLibraryOpen(false); drawing.reset(); await loadProject(created); return created; } catch (error) { setOperationError(safePersistenceError(error).reason); return undefined; } }
  async function duplicateProject(id: string) { const live = projectNavigation.getSession()?.getViewState().project; const source = live?.id === id ? live : autosave.latest(id) ?? projects.find((project) => project.id === id); if (!source) return; try { const duplicate = await saveProject({ ...structuredClone(source), id: crypto.randomUUID(), name: `${source.name} — ${locale === "pl" ? "kopia" : "copy"}` }); setProjects((current) => [duplicate, ...current]); return duplicate; } catch (error) { setOperationError(safePersistenceError(error).reason); return undefined; } }
  async function deleteProject(id: string) { try { await autosave.remove(id, (expectedRevision) => removeProject(id, expectedRevision)); let remaining = projects.filter((project) => project.id !== id); if (!remaining.length) remaining = [await saveProject(createProjectAtScale(crypto.randomUUID(), locale === "pl" ? "Nowy projekt" : "New project", locale, "world"))]; setProjects((current) => [...current.filter((project) => project.id !== id), ...remaining.filter((project) => !current.some(({ id }) => id === project.id))]); setPendingDeleteId(undefined); if (projectNavigation.getSession()?.getViewState().project.id === id && await loadProject(remaining[0])) drawing.reset(); return true; } catch (error) { setOperationError(safePersistenceError(error).reason); return false; } }
  async function renameProjectInLibrary(id: string, name: string) { try { const active = projectNavigation.getSession(); if (active?.getViewState().project.id === id) { if (!workbenchTransaction.commit(`rename-project:${id}`, (project) => ({ ...project, name: name.trim() }))) return; const result = await autosave.flush(active.getViewState().project); if (result.state === "saved") return result.project; if (result.state === "failed") setOperationError(`${copy.saveFailed} [${result.error.code}] ${result.error.reason}`); else setOperationError(locale === "pl" ? "Projekt zmieniono w innej karcie. Otwórz go ponownie." : "The project changed in another tab. Reopen it."); return undefined; } const source = autosave.latest(id) ?? projects.find((project) => project.id === id); if (!source) return; const result = await autosave.flush({ ...source, name: name.trim() }); if (result.state === "saved") return result.project; if (result.state === "failed") setOperationError(`${copy.saveFailed} [${result.error.code}] ${result.error.reason}`); else setOperationError(locale === "pl" ? "Projekt zmieniono w innej karcie. Otwórz go ponownie." : "The project changed in another tab. Reopen it."); return undefined; } catch (error) { setOperationError(safePersistenceError(error).reason); return undefined; } }
  async function openSavedProject(id: string) { try { const live = projectNavigation.getSession()?.getViewState().project; const project = live?.id === id ? live : autosave.latest(id) ?? projects.find((candidate) => candidate.id === id); if (!project) return false; const opened = await loadProject(project); if (opened) drawing.reset(); return opened; } catch (error) { setOperationError(safePersistenceError(error).reason); return false; } }
  const { exportProject, exportView, importProject } = projectLibraryFileActions({ snapshot, projects, getActiveProject: () => projectNavigation.getSession()?.getViewState().project, locale, viewport, onError: setOperationError, onImport: (project) => setProjects((current) => [project, ...current]) });
  ...
  const enterStory = () => { drawing.leaveDrawing(); if (session) setSnapshot(session.getViewState()); setMode("story"); };
  ...
  const inspectedPlaceId = currentProject ? resolvedInspectedPlaceId(currentProject, inspectorTarget, activePlaceId) : undefined;
  const story = useWorkbenchStory({ session, snapshot, selections, inspectedPlaceId, locale, mode, refresh: () => refresh(), persistNotebook: async (project) => (await autosave.saveStoryDocuments(project)).state === "saved", zoom: viewport.zoom, onSelect: selectOnMap, onOpenPlace: commitOpenPlace, onFocus: createStoryObjectFocus(() => session?.getViewState(), commitOpenPlace, setSelections), onOpenWorldbook: () => drawing.requestAfterDraft(() => requestAfterOverlap(enterStory)) });
  const [agentChange, setAgentChange] = useState<AgentChangeReport>();
  const readProposalChanges = useProposalChangeReader(session);
  const projectCheckpoints = useProjectCheckpoints(currentProject, locale, () => projectNavigation.getSession()?.getViewState().project); useEditorV2Tools(session, activePlaceId, { getEditorContext: () => story.liveContext, setStoryView: (view) => requestStoryViewTransition({ view, drawing, hasOverlap: () => { const current = session?.getState(); const active = current?.activePlaceId; return Boolean(current && active && buildingOverlapGroups(current.project, active).length); }, requestAfterOverlap: (action) => requestAfterOverlap(action), setMode: enterStory, setStoryView: story.setView }), preserveAgentChange: projectCheckpoints.preserveAgentChange, reportAgentChange: setAgentChange, refresh: () => refresh(), openPlace: (placeId) => { if (!session) return false; const exists = session.getState().project.places.some(({ id }) => id === placeId); if (exists) commitOpenPlace(placeId); return exists; }, focusObjects: (refs) => createStoryObjectFocus(() => session?.getViewState(), commitOpenPlace, setAgentFocus)(refs.map(({ type, ...ref }) => ({ ...ref, kind: type }))), clearFocus: () => setAgentFocus([]), getCheckpoints: () => projectCheckpoints.items, createCheckpoint: (name) => projectCheckpoints.preserve(name), deleteCheckpoint: async (id) => { await projectCheckpoints.remove(id); return true; }, showCheckpoint: (checkpointId, opacity) => { if (checkpointId && !projectCheckpoints.items.some(({ id }) => id === checkpointId)) return false; projectCheckpoints.setActiveId(checkpointId); if (opacity !== undefined) projectCheckpoints.setOpacity(opacity); return true; }, getProjects: () => projects, createProject: (name, scale) => createProject({ name, scale }), openProject: openSavedProject, duplicateProject, renameProject: renameProjectInLibrary, deleteProject }, locale);
