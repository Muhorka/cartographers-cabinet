...
type Props = {
  ...
  onUpdateTransition?(selection: MapSelection, details: Partial<Pick<VerticalTransition, "kind" | "sourceLevelId" | "targetLevelId" | "connectedLevelIds" | "style" | "direction" | "sameLevelRise">>): void;
...
function SelectionForm(props: Props & { selection: MapSelection }) {
  ...
  const construction = (selection.kind === "room" || selection.kind === "wall" || selection.kind === "opening" || selection.kind === "transition") ? constructionForSelection(project, selection) : project.constructions.find(({ id }) => id === constructionOwner?.constructionId);
  ...
  if (selection.kind === "opening") {
    if (!construction) return <p className={styles.empty}>{copy.noSelection}</p>;
    ...
    const index = construction.openings.findIndex(({ id }) => id === opening.id); const fallback = copy.objectList.openingName(opening.kind, index + 1); const label = createProjectStoryLabelResolver(project)({ kind: "opening", id: opening.id, scopeId: construction.id }, fallback);
    return <div className={styles.form}><strong className={styles.selectionKind}>{label}</strong><div className={styles.belongs}><span>{copy.type}</span><strong>{copy.openingTypes[opening.kind]}</strong></div><label><span>{copy.width}</span><input disabled={props.readOnly} type="number" min="0.2" step="0.1" value={opening.width} onChange={(event) => { const width = Number(event.currentTarget.value); if (Number.isFinite(width) && width > 0 && width !== opening.width) props.onResizeOpening(selection, width); }}/></label></div>;
  }
  const transitionDocument = selection.scopeId ? construction : construction?.transitions.some(({ id }) => id === selection.id) ? construction : project.constructions.find((candidate) => candidate.transitions.some(({ id }) => id === selection.id));
  if (!transitionDocument) return <p className={styles.empty}>{copy.noSelection}</p>;
  ...
  const transitionIndex = transitionDocument.transitions.findIndex(({ id }) => id === transition.id); const transitionFallback = transition.kind === "elevator" ? copy.objectList.elevatorName?.(transitionIndex + 1) ?? copy.objectList.stairsName(transitionIndex + 1) : copy.objectList.stairsName(transitionIndex + 1); const transitionLabel = createProjectStoryLabelResolver(project)({ kind: "transition", id: transition.id, scopeId: transitionDocument.id }, transitionFallback);
  ...
  return <div className={styles.form}><strong className={styles.selectionKind}>{transitionLabel}</strong><label><span>{copy.type}</span><select disabled={props.readOnly} value={transition.kind} onChange={(event) => props.onUpdateTransition?.(selection, { kind: event.currentTarget.value as VerticalTransition["kind"] })}><option value="stairs">{copy.stairs}</option><option value="elevator">{copy.elevator}</option></select></label>{transition.kind === "stairs" && <><label><span>{copy.stairStyle}</span><select disabled={props.readOnly} value={transition.style ?? "straight"} onChange={(event) => props.onUpdateTransition?.(selection, { style: event.currentTarget.value as NonNullable<VerticalTransition["style"]> })}>{Object.entries(copy.stairStyles).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span>{copy.direction}</span><input disabled={props.readOnly} type="number" min="-360" max="360" step="15" value={transition.direction ?? 0} onChange={(event) => props.onUpdateTransition?.(selection, { direction: Number(event.currentTarget.value) })}/></label></>}<label><input disabled={props.readOnly} type="checkbox" checked={Boolean(transition.sameLevelRise)} onChange={(event) => props.onUpdateTransition?.(selection, { sameLevelRise: event.currentTarget.checked, connectedLevelIds: event.currentTarget.checked ? sourceLevel ? [sourceLevel.id] : [] : [...connected] })}/><span>{copy.sameLevelRise}</span></label>{!transition.sameLevelRise && levels.length > 0 && <TransitionLevelChecklist levels={levels} connected={connected} sourceLevelId={sourceLevel?.id} label={copy.connectsLevels} disabled={props.readOnly} onChange={(connectedLevelIds) => props.onUpdateTransition?.(selection, { connectedLevelIds, sourceLevelId: sourceLevel?.id, targetLevelId: connectedLevelIds.find((id) => id !== sourceLevel?.id) })}/>} </div>;
}
