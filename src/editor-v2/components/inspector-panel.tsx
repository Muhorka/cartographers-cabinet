...
function SelectionForm(props: Props & { selection: MapSelection }) {
  ...
  const transitionDocument = construction?.transitions.some(({ id }) => id === selection.id) ? construction : project.constructions.find((candidate) => candidate.transitions.some(({ id }) => id === selection.id));
