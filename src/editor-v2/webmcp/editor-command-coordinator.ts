...
type PendingChange = PreparedChange & { baseRevision: string; transactionId: string; sessionToken: symbol; preparedTransaction: ReadyTransaction; createdAt: number };
...
export class EditorCommandCoordinator {
  ...
  private purge(now: number) {
    ...
    this.removeOldest(this.pending, this.maxPending);
    this.removeOldest(this.applied, this.maxApplied);
  }

  private removeOldest<T extends { createdAt: number }>(entries: Map<string, T>, max: number) {
  ...
  revision() {
    return projectRevision(this.maintain().getState().project);
  }

  prepare(transactionId: string, build: (project: EditorProject) => PreparedChange) {
    const session = this.maintain(); const before = session.getState().project; const baseRevision = projectRevision(before); const createdAt = this.now();
    ...
    try {
      ...
      const preparedTransaction = session.prepareTransaction({ id: transactionId, apply: () => change.project });
      ...
      this.pending.set(token, { ...change, baseRevision, transactionId, sessionToken: this.getSessionToken(session), preparedTransaction, createdAt });
      this.removeOldest(this.pending, this.maxPending);
