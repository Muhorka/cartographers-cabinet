...
export function buildRoadEdit(project: EditorProject, value: unknown) {
  ...
  return { project: next, summary: "Updated road shape / width.", effects: [`updated:element:${road.id}`] };
}

export function buildRoadJoin(project: EditorProject, refs: AgentObjectRef[]) {
  ...
  return { project: result.project, summary: result.state === "joined" ? "Połączono dwie drogi w jeden edytowalny przebieg." : `Utworzono ${result.junctions.length} skrzyżowanie(a) dróg.`, effects: result.state === "joined" ? [`joined:road:${result.survivorId}:${result.removedId}`] : result.junctions.map(({ id }) => `junction:road:${id}`) };
}
