...
export function createStoryRouteTools(bridge: EditorAgentBridge & EditorContextBridge, coordinator: EditorCommandCoordinator, suppliedRouteService?: ReturnType<typeof createStoryRouteCalculationService>): WebMcpTool[] {
  ...
  return [
    { name: "find_story_routes", description: "Calculate paths over openings, floor connections, and outdoor geometry, respecting narrative access and traveller width. Endpoint coordinates are LOCAL to each place (metres); defaults to active scenario/step. Vehicles use roads by default; foot/mounted may go offroad. Return ready/unreachable/unknown with source revision, conditions, and up to 3 alternatives; timeout/cancel/error/stale are not inaccessibility evidence. Worker limit 20s; newer requests cancel older ones. Unknown guards/secrets remain missing facts. Read-only.", inputSchema: z.toJSONSchema(routeQuery, { io: "input" }), annotations: { readOnlyHint: true }, execute: async (raw) => {
      const { query } = routeQuery.parse(raw); return calculate(capture(query));
    } },
    { name: "prepare_save_story_route", description: "Calculate and preserve a named route with its actual result/source revision, never a caller path. Geometry/narrative changes make it stale; timeout, cancellation, error, or stale context never saves. Saved records do not invalidate the calculation. Commit via execute_editor_batch.", inputSchema: z.toJSONSchema(saveQuery, { io: "input" }), annotations: { readOnlyHint: false }, execute: async (raw) => {
