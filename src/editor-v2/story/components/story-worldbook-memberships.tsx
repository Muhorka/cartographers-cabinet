...
export function StoryWorldbookMemberships({ entry, story, copy, onChange, omitKnowledge = false }: { entry: StoryRecord; story: StoryDocumentLike; copy: StoryCopy; onChange(entry: StoryRecord): void; omitKnowledge?: boolean }) {
  ...
  return <div className={styles.optionalSections}>{definitions.filter(({ field }) => !omitKnowledge || field !== "knownEntryIds").map(({ field, label, hint, empty, options }) => {
