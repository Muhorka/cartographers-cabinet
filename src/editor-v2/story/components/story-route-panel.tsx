...
export function StoryRoutePanel({ project, activePlaceId, locale, context, initialRoute, onDelete, onPreview, onSave, onOpenPlace, onRequestPoint, routeService }: StoryRoutePanelProps) {
  ...
  return <section className={styles.panel} aria-label={pl ? "Planowanie tras" : "Route planning"}>
    ...
    <p>{pl ? "Wybierz miejsce lub teren i wskaż punkt na mapie. Trasa uwzględnia ściany, przejścia i dostęp postaci." : "Choose a place or terrain object and pick a point on the map. Routes respect walls, passages and character access."}</p>
    ...
    {result && <div role="status"><strong>{stale ? pl ? "Plan się zmienił — przelicz trasę." : "The plan changed — recalculate." : result.result.status === "ready" ? pl ? "Znalezione warianty" : "Route alternatives" : result.result.status === "unknown" ? pl ? "Brakuje danych do potwierdzenia trasy" : "Missing facts prevent verification" : pl ? "Nie znaleziono dostępnej trasy" : "No accessible route found"}</strong>
