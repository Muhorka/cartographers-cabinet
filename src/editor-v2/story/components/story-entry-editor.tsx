...
function StoryDescription({ entry, copy, onChange }: { entry: StoryRecord; copy: StoryCopy; onChange(next: StoryRecord): void }) {
  const help = worldbookHelp(copy);
  return <details className={styles.subsection} open>
    <summary>{copy.descriptionAndKnowledge}</summary>
    <label className={styles.field}><span>{help.optionalDescription}</span><textarea data-textarea-size-key={`story-entry:${entry.id}:description`} value={String(entry.description ?? "")} placeholder={help.descriptionPlaceholder} onChange={(event) => onChange({ ...entry, description: event.currentTarget.value })} rows={4}/><p className={flow.hint}>{help.descriptionHint}</p></label>
  </details>;
}
...
export function StoryEntryEditor({ entry, collection, story, copy, resolvedObjects, onChange, onRemove }: { entry: StoryRecord; collection: StoryCollection; story: StoryDocumentLike; copy: StoryCopy; resolvedObjects: StoryResolvedObject[]; onChange(next: StoryRecord): void; onRemove(): void }) {
  ...
  return <div className={styles.form}>
    ...
    {(["keys", "objectGroups", "zones", "scenarios"] as StoryCollection[]).includes(collection) && <label className={styles.field}><span>{help.optionalDescription}</span><textarea data-textarea-size-key={`story-entry:${entry.id}:description`} value={String(entry.description ?? "")} onChange={(event) => update("description", event.currentTarget.value)} rows={3}/></label>}
    {(["characters", "factions", "accessGroups"] as StoryCollection[]).includes(collection) && <><p className={flow.hint}>{help.optional}</p><StoryDescription entry={entry} copy={copy} onChange={onChange}/><StoryWorldEntryTraits entry={entry} story={story} resolvedObjects={resolvedObjects} copy={copy} onChange={onChange}/><StoryEntryConnections entry={entry} story={story} resolvedObjects={resolvedObjects} copy={copy}/><StoryWorldbookMemberships entry={entry} story={story} copy={copy} onChange={onChange}/></>}
