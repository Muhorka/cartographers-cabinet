...
export function StoryEntryEditor({ entry, collection, story, copy, resolvedObjects, onChange, onRemove }: { entry: StoryRecord; collection: StoryCollection; story: StoryDocumentLike; copy: StoryCopy; resolvedObjects: StoryResolvedObject[]; onChange(next: StoryRecord): void; onRemove(): void }) {
  ...
  const projectObjectOptions = storyObjectOptions(story, resolvedObjects);
  ...
  const relationOptions = [...story.world.map((item) => ({ id: `entryId:${item.id}`, name: item.name, source: "characters" as StoryCollection })), ...projectObjectOptions];
  ...
  return <div className={styles.form}>
    {collection !== "intentions" && <label className={styles.field}><span>{labels.name}</span><input required value={entry.name} onChange={(event) => update("name", event.currentTarget.value)}/></label>}
    {(["characters", "factions", "accessGroups", "keys", "objectGroups", "zones", "scenarios"] as StoryCollection[]).includes(collection) && <label className={styles.field}><span>{help.optionalDescription}</span><textarea value={String(entry.description ?? "")} onChange={(event) => update("description", event.currentTarget.value)} rows={3}/></label>}
    {(["characters", "factions", "accessGroups"] as StoryCollection[]).includes(collection) && <><p className={flow.hint}>{help.optional}</p><StoryEntryConnections entry={entry} story={story} resolvedObjects={resolvedObjects} copy={copy}/><StoryWorldbookMemberships entry={entry} story={story} copy={copy} onChange={onChange}/></>}
    ...
    {collection === "relations" && <><label className={styles.field}><span>{copy.from}</span><select value={refs("fromRefs")[0] ?? ""} onChange={(event) => update("fromRefs", event.currentTarget.value)}>{relationOptions.filter(({ id }) => id !== entry.id).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className={styles.field}><span>{copy.to}</span><select value={refs("toRefs")[0] ?? ""} onChange={(event) => update("toRefs", event.currentTarget.value)}>{relationOptions.filter(({ id }) => id !== entry.id).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className={styles.field}><span>{copy.relationKind}</span><select value={String(entry.kind ?? "custom")} onChange={(event) => update("kind", event.currentTarget.value)}>{["owns", "knows", "visits", "guards", "uses", "contains", "custom"].map((kind) => <option key={kind} value={kind}>{copy[kind] ?? kind}</option>)}</select></label><label className={styles.field}><span>{copy.source}</span><input value={String(entry.source ?? "")} onChange={(event) => update("source", event.currentTarget.value)}/></label></>}
