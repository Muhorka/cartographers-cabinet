...
const pl: Partial<Record<StoryCollection, EntryCopy>> = {
  ...
  accessGroups: { add: "Dodaj grupę dostępu", create: "Utwórz grupę dostępu", name: "Nazwa grupy dostępu", hint: "Na przykład goście, służba lub straż. Takiej grupie możesz pozwolić wchodzić do wybranych miejsc." },
...
const en: Partial<Record<StoryCollection, EntryCopy>> = {
  ...
  accessGroups: { add: "Add access group", create: "Create access group", name: "Access group name", hint: "For example guests, servants or guards. Grant a group access to selected places." },
  ...
  relations: { add: "Add relation", create: "Create relation", name: "Relation name", hint: "Record connections in your world, such as who visits or guards a place. They help organize the story; your agent can read them as context. A relation alone does not grant access, change ownership or trigger character actions." },
...
const worldbookCommonCopy = {
  pl: {
    ...
    relationHint: "Podaj nazwę i wybierz obie strony powiązania.", intentionHint: "Opisz zamysł i wybierz obiekt, którego dotyczy.",
    ...
    groups: "Do jakich grup należy „{name}”?", groupsHint: "Wybierz frakcje lub grupy dostępu, np. Straż albo Goście. To przynależność tej postaci lub organizacji, nie miejsca na mapie.",
    ...
    knowledge: "O kim lub o czym wie „{name}”?", knowledgeHint: "Wybierz znane postacie, frakcje lub inne wpisy księgi. To wiedza, nie prawo wstępu.",
    noGroups: "Nie ma jeszcze frakcji ani grup dostępu. Dodaj je w odpowiedniej kategorii księgi, a potem wybierz tutaj.",
  ...
  en: {
    ...
    creationHint: "Only a name is required. Add other details later.", relationHint: "Name the relation and select both sides.", intentionHint: "Describe your intent and select its subject.",
    ...
    groups: "Which groups does “{name}” belong to?", groupsHint: "Choose factions or access groups, such as Guards or Guests. This is membership of the person or organization, not a map location.",
    keys: "Which keys does “{name}” hold?", keysHint: "Select the keys they possess. Routes can use these when assessing locked doors.",
    knowledge: "Who or what does “{name}” know about?", knowledgeHint: "Choose known people, factions or other book entries. Knowledge does not grant access.",
    noGroups: "No factions or access groups yet. Add them in the matching book category, then select them here.", noKeys: "No keys yet. Add them in Keys, then select them here.", noKnowledge: "No other entries yet. You can leave this section empty.",
