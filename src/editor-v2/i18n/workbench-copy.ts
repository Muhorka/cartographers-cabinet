...
export const workbenchCopy: Record<EditorLocale, WorkbenchCopy> = {
  pl: {
    ...
    map: { ariaLabel: "Arkusz kartograficzny", empty: "Ten arkusz jest jeszcze pusty", compass: "Obróć mapę", zoomIn: "Przybliż mapę", zoomOut: "Oddal mapę", resetView: "Wyzeruj widok", back: "Wróć do szerszej mapy", northMark: "P", measurements: { title: "Widok i miary", grid: "Siatka pomocnicza", axes: "Pokaż osie", opacity: "Przezroczystość", spacing: "Odstęp", cell: "1 kratka", snap: "Przyciągaj do siatki", units: "Jednostki", metric: "metry", imperial: "stopy", roomAreas: "Powierzchnie obiektów" }, openingLabel: (kind, _id, index) => `${({ door: "Drzwi", window: "Okno", gate: "Brama", passage: "Przejście" })[kind]} ${index}`, transitionLabel: (_id, kind, index) => `${kind === "elevator" ? "Winda" : "Schody"} ${index}` },
    drawingStatus: {
  ...
  en: {
    ...
    map: { ariaLabel: "Cartographer's sheet", empty: "This sheet is still empty", compass: "Rotate map", zoomIn: "Zoom in", zoomOut: "Zoom out", resetView: "Reset view", back: "Return to the broader map", northMark: "N", measurements: { title: "View & measurements", grid: "Grid", axes: "Show axes", opacity: "Opacity", spacing: "Spacing", cell: "1 cell", snap: "Snap to grid", units: "Units", metric: "metres", imperial: "feet", roomAreas: "Object areas" }, openingLabel: (kind, _id, index) => `${({ door: "Door", window: "Window", gate: "Gate", passage: "Passage" })[kind]} ${index}`, transitionLabel: (_id, kind, index) => `${kind === "elevator" ? "Lift" : "Stairs"} ${index}` },
    drawingStatus: {
