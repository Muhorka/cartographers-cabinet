...
export type ProjectLibraryCopy = {
  ...
  recoveryWarning: (count: number) => string; recoveryOnlyTitle: string; recoveryOnlyBody: (count: number) => string; createNewProject: string; exportRecovery: string; recoveryKey: string; recoverySafety: string;
};
export const projectLibraryCopy: Record<EditorLocale, ProjectLibraryCopy> = {
  ...
  en: {
    ...
    recoveryWarning: (count) => `Some saved projects need recovery (${count}). The remaining projects were loaded. Damaged data was not deleted.`, recoveryOnlyTitle: "No valid project could be loaded", recoveryOnlyBody: (count) => `${count} damaged saved ${count === 1 ? "project was" : "projects were"} found. Export an emergency copy first.`, createNewProject: "Create a new project", exportRecovery: "Export emergency copy", recoveryKey: "Dexie key", recoverySafety: "No repair or deletion was performed automatically.",
  },
};
